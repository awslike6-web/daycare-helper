...
- 3. 월간 발달 관찰기록부 (monthly_observation) [🚨 무결성 가드 - 가짜 행동 지어내기(할루시네이션) 원천 차단]:
  ...
  * play_obs (1차 [놀이] 관찰, 상순: ${obsDate1}, 영역: ${obsArea1}): 오늘 입력된 실제 메모 팩트만을 바탕으로 사실적이고 객관적인 행동과 교사의 즉각적 비계설정/지원을 서술하세요.
  * daily_obs (2차 [일상생활] 관찰): 🚨 절대 가상의 사건이나 없는 놀이 행동을 소설 쓰듯 지어내지 마세요(할루시네이션 엄격 금지)!
...
- 3. 월간 발달 관찰기록부 (monthly_observation) [⭐ 실제 팩트 기반 시계열 연속 관찰 모드]:
  ...
  * 제공된 이전 기록 중 적절한 사건을 play_obs(1차 [놀이], ${obsDate1})에 매핑하고, 오늘의 최신 사건을 daily_obs(2차 [일상생활], ${obsDate2})에 매핑하여, 1차 대비 2차에서의 실제 발전적 변화(growth_continuity)를 완벽하게 도출하세요.
