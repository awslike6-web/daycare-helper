...
## 7. 인프라 및 보안 접근 제어 (Cloudflare Zero Trust Access)

외부 비인가자의 접속 차단, Gemini API 사용량(비용) 남용 방어, 아동 개인정보의 완전 밀폐를 위해 **Cloudflare Zero Trust Access**를 단일 게이트웨이로 적용합니다.

### 1) 인증 및 접근 정책
- **인증 방식**: One-Time PIN (이메일 화이트리스트 6자리 OTP 인증)
- **허용 대상 (Allowlist)**: 관리자, 아내, 처형의 승인된 이메일 주소만 허용
- **세션 유지 기간 (Session Duration)**: **1개월 (1 month / 약 730시간)**
  - 보육 현장의 접속 피로도를 최소화하기 위해 최초 1회 인증 후 30일간 일반 웹사이트처럼 즉시 접속 가능

### 2) 세션 만료 7일 전(D-7) 사전 안내 및 자율 해제(Dismiss) 시스템
- **동작 메커니즘**:
  1. Cloudflare Worker가 요청 헤더의 `Cf-Access-Jwt-Assertion`을 감지하여 JWT 페이로드에서 만료 시각(`exp`)을 추출
  2. 만료까지 남은 일수가 **7일 이하(`remainingDays <= 7`)**일 때 웹 프론트엔드 최상단에 다정한 안내 배너 자동 팝업
     > 🔐 **보안 세션 만료 D-6일 전이에요!** 곧 접속 시 이메일 6자리 인증이 다시 뜰 수 있어요.
  3. **자율 해제 (Dismiss)**:
     - 교사가 **`[알겠어요 / 확인 완료]`** 버튼을 클릭하면 `localStorage`에 해당 세션 만료 타임스탬프가 기록되며, 해당 세션 만료 시점까지 배너가 깔끔하게 숨김 처리됨
     - 30일 경과 후 실제로 재인증을 받으면 새로운 세션 `exp`가 부여되어 만료 7일 전까지는 배너가 다시 뜨지 않음
  4. **⚙️ 설정 창 상시 조회**:
     - ⚙️ 설정 모달 내에서 현재 로그인된 이메일 계정과 잔여 세션 기간을 실시간으로 확인 가능
